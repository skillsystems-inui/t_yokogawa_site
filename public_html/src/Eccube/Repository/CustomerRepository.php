...
class CustomerRepository extends AbstractRepository
{
    ...
    public function getQueryBuilderBySearchData($searchData)
    {
        ...
        if (isset($searchData['multi']) && StringUtil::isNotBlank($searchData['multi'])) {
            ...
            $qb
                ->andWhere('c.id = :customer_id OR c.customer_code = :customer_code OR CONCAT(c.name01, c.name02) LIKE :name OR CONCAT(c.kana01, c.kana02) LIKE :kana OR c.email LIKE :email')
                ->setParameter('customer_id', $id)
                ->setParameter('customer_code', $clean_key_multi)
        ...
        // familymain 自身が家族代表であるか　代表である:1 代表でない:1以外
        if (!empty($searchData['customer_familymain']) && count($searchData['customer_familymain']) > 0) {
            $qb
                ->andWhere($qb->expr()->in('c.Familymain', ':familymain'))
                ->setParameter('familymain', $searchData['customer_familymain']);
        }
    ...
    {
        $maxCustomerCode = $this->createQueryBuilder('c')
                    ->select('COALESCE(MAX(c.customer_code), 0)')
                    ->getQuery()
