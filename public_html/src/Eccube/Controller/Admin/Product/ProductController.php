...
class ProductController extends AbstractController
{
    ...
    public function edit(Request $request, $id = null, RouterInterface $router, CacheUtil $cacheUtil)
    {
        ...
        //クラス取得(オプション用)
        $class_categories = [];
        $ProductClassCategories = $Product->getProductClassCategories();
        
        
        foreach ($ProductClassCategories as $ProductClassCategory) {
            /* @var $ProductClassCategory \Eccube\Entity\ProductClassCategory */
            
            $ClassName = $ProductClassCategory->getClassCategory()->getClassName();
            
            //$class_categories[] = $ClassName->getClassCategories();
            //$class_categories[] = $ProductClassCategory->getClassCategory();
            $class_categories[] = $ClassName;
            
        
        }
        $form['ClassName']->setData($class_categories);
        ...
        if ('POST' === $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                /* @var $Product \Eccube\Entity\Product */
                // 子オプションをクリア
                foreach ($Product->getProductClassCategories() as $ProductClassCategory) {
                    $Product->removeProductClassCategory($ProductClassCategory);
                    $this->entityManager->remove($ProductClassCategory);
                }
                // 親オプションをクリア
                foreach ($Product->getProductClassNames() as $ProductClassName) {
                    $Product->removeProductClassName($ProductClassName);
                    $this->entityManager->remove($ProductClassName);
                }
                $this->entityManager->persist($Product);
                $this->entityManager->flush();

                $class_count = 1;
                $ClassNames = $form->get('ClassName')->getData();
                
                $classCategoriesIdList = [];
                
                //ProductClassNameテーブル用キーチェック
                $exist_product_id = 0;
                $exist_classname_id = 0;
                //選択した親オプション単位のループ
                foreach ($ClassNames as $ClassNameParent) {
                    
                    
                    
                    //親オプションを登録
                    //ClassName登録
                    $ProductClassName = $this->createProductClassName($Product, $ClassNameParent);
                    
                    //ProductClassNameテーブル用キーチェック(現行)
                    $temp_product_id = $ProductClassName->getProductId();
	                $temp_classname_id = $ProductClassName->getClassNameId();
                    
                    //キーの重複チェック
                    if( !(($temp_product_id == $exist_product_id) && ($temp_classname_id == $exist_classname_id)) ){
	                    $this->entityManager->persist($ProductClassName);
	                    //$this->entityManager->flush();
	                    /* @var $Product \Eccube\Entity\Product */
	                    $Product->addProductClassName($ProductClassName);
	                    
	                    //ProductClassNameテーブル用キーチェック(既存)
		                $exist_product_id = $ProductClassName->getProductId();
	                    $exist_classname_id = $ProductClassName->getClassNameId();
                    
                    }
                    //.親オプションを登録
                    
                    //選択した子オプション単位のループ
                    foreach ($ClassNameParent->getClassCategories() as $ClassNameChild) {
                        //子オプションを登録
	                    if (!isset($classCategoriesIdList[$ClassNameChild->getId()])) {
	                        //ClassCategory登録
	                        $ProductClassCategory = $this->createProductClassCategory($Product, $ClassNameChild, $class_count);
	                        $this->entityManager->persist($ProductClassCategory);
	                        $class_count++;
	                        /* @var $Product \Eccube\Entity\Product */
	                        $Product->addProductClassCategory($ProductClassCategory);
	                        $classCategoriesIdList[$ClassNameChild->getId()] = true;
	                    }
        ...
        //オプション用に規格を取得(サイズ以外)
        $TopClassNames = $this->classNameRepository->getListWithoutSize();
        
        $ChoicedClassIds = array_map(function ($ClassName) {
            return $ClassName->getId();
        }, $form->get('ClassName')->getData());
        ...
        return [
            ...
            'ChoicedClassIds' => $ChoicedClassIds,
        ];
    }
            ...
            $this->csvExportService->exportData(function ($entity, CsvExportService $csvService) use ($request) {
                ...
                foreach ($ProductClasses as $ProductClass) {
                    ...
                    foreach ($Csvs as $Csv) {
                        ...
                        if ($ExportCsvRow->isDataNull() && $RelatedProducts = $Product->getRelatedProducts()) {
                            // 商品規格情報になければ関連商品情報を検索.
                            $ExportCsvRow->setData($csvService->getData($Csv, $RelatedProducts));
                        }
     ...
     * @param integer $count
     *
     * @return \Eccube\Entity\ProductClassCategory
     */
    private function createProductClassCategory($Product, $ClassCategory, $count)
    {
     ...
     * @return \Eccube\Entity\ProductClassCategory
     */
    private function createProductClassCategory__($Product, $ClassCategory, $count)
    {
        $ProductClassCategory = new ProductClassCategory();
        $ProductClassCategory->setProduct($Product);
        $ProductClassCategory->setProductId($Product->getId());
     ...
     *
     * @return \Eccube\Entity\ProductClassName
     */
    private function createProductClassName($Product, $ClassName)
    {
